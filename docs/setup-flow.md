...
5. `ssh_cnote(action=get|set|append, profile_id=...)` — read or update the profile's Cnote when the user wants persistent AI instructions.
6. `ssh_connect(profile_id=...)` — establishes the SSH connection and returns the current `cnote`.
7. Use operational tools (`ssh_exec`, `ssh_transfer`, `ssh_read_file`, `ssh_write_file`, `ssh_apply_patch`) as needed.
